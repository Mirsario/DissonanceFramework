...
namespace Dissonance.Framework.GLFW3
{
	partial class GLFW
	{
		...
		[UFP(CC.Cdecl)] public delegate void MouseButtonCallback(IntPtr window,int button,int action,int mods);
		...
		[UFP(CC.Cdecl)] public delegate void KeyCallback(IntPtr window,int key,int scanCode,int action,int mods);
